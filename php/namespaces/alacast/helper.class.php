<?php
	...
	class helper {
		...
		static public function preg_match_array(array &$haystack_array, $needle_preg_match, $preg_replace=NULL ) {
			$argc=count($haystack_array);
			for($i=0; $i<$argc; $i++)
				if( (preg_match( $needle_preg_match, $haystack_array[$i] )) ){
					if(!$preg_replace)
						return TRUE;
					
					$preg_match="";
					if(($preg_match=preg_replace($needle_preg_match, $preg_replace, $haystack_array[$i]))!="")
						return $preg_match;
						//return ($haystack_array[$i]=$preg_match);
					else
						return FALSE;
				}

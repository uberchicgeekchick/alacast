...
check_dependencies:
	...
	set dependencies=("${scripts_basename}");# "`printf '%s' '${scripts_basename}' | sed -r 's/(.*)\.(tcsh|cshrc)${eol}/\1/'`");
	@ dependencies_index=0;
#check_dependencies:


check_dependency:
	set current_label="check_dependency";
...
main:
	...
	if(! ${?playlist_dir} ) then
		...
		if( ${?alacast_ini} ) then
			set playlist_dir="`/bin/grep --perl-regexp '.*playlist_dir=.*' '${alacast_ini}' | /bin/sed -r 's/.*playlist_dir[^=]*=["\""'\'']([^"\""'\'']*)["\""'\''];/\1/'`";
			unset alacast_ini;
		endif
	endif
#main:
...
exec:
	...
	if( "${EDITOR}" == "vim" || "${EDITOR}" == "vim-enhanced" ) then
		set editor_command="${EDITOR} -p";
	else
		set editor_command="${EDITOR}";
	endif
	set playlist="`/bin/ls -t --width=1 "\""${playlist_dir}/"\"" | head -1`";
	if(!( "${playlist}" != "" && -e "${playlist_dir}/${playlist}" )) then
		printf "**%s error:** No current playlist could be found.\n" "${scripts_name}" > /dev/stderr;
	else
		echo $editor_command "${playlist_dir}/${playlist}" $argz;
	endif
	unset editor_command playlist_dir playlist;
	goto scripts_main_quit;
#exec:

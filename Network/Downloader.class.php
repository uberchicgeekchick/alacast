<?php
	 ...
	 *	Find joy!
	 */
	namespace Alacast::Network;

	class Downloader extends Alacast::Network{
		public $Public_Variale;
		...
		private function Clear_Numbers(&$Titles){
			$Total_Titles=count($Titles);
			for($i=0; $i<$Total_Titles; $i++)
				$Titles[$i]=preg_replace("/^([^0-9])([0-9])/", "0$1$2", (
					preg_replace("/\([^0-9]\)\([0-9]\)\([^0-9]\)/", "\10\2\3/", (
						preg_replace( "/\([^0-9]\)\([0-9]\)$/", "\10\2/", $Titles[$i] )
					)
				);
		}//Clear_Numbers

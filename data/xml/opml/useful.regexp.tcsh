...
#		1,$s/\v\<title\>([^<]+)\<\/title\>\n\<pubDate\>([^<]*)\<.*\n\<enclosure (url|href)\="(.*)(\.[^."?]+)(\??[^"]*)".*/wget -O "\1, released on: \2\5" "\4\5\6";/
#		1,$s/\v\<title\>([^<]+)\<\/title\>\n\<enclosure (url|href)\="(.*)(\.[^."?]+)(\??[^"]*)".*\n\<pubDate\>([^<]*)\<.*/wget -O "\1, released on: \6\4" "\3\4\5";/
#	
#	Or for a podcast novel, autio drama, or podiobook:
#	
#	w/o pubDates:
#		'+1,$s/\v\<title\>([^0-9]+ )([0-9]+)\<\/title\>\n.*\n.*\n\<enclosure (url|href)\="(.*)(\.[^."?]+)(\??[^"]*)".*/wget -O "\1\- Episode \2\5" "\4\5\6";'/
#	
#	w/pubDates:
#		'+1,$s/\v\<title\>([^0-9]+ )([0-9]+)\<\/title\>\n(\<pubDate\>)?([^<]*)\<.*\n(\<pubDate\>)?([^<]*)\<.*\n\<enclosure (url|href)\="(.*)(\.[^."?]+)(\??[^"]*)".*/wget -O "\1\- Episode \2, released on: \3\5\8" "\7\8\9";'/
#
#-----------------------------------------------------------------------------------
